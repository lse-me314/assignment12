...
In theory,[^theory] you shouldn't need anything other than a Twitter account to be able to use the `rtweet` package. If you don't have a Twitter account: 1) Congratulations, you must be more productive than me, and 2) You will need to open one to interact with the API (sorry). In order to access the API, you will need to be logged in to Twitter on your browser, and then you will need to execute the following command and follow the on-screen instructions:
...
Let's start by collecting some recent Tweets about [EURO2022](https://www.uefa.com/womenseuro/):

```{r, eval = TRUE, echo = FALSE}

load("data/euro_tweets.Rdata")

```

```{r, eval = FALSE, echo = TRUE}

euro_tweets <- search_tweets("#WEURO2022", n = 200)
...
Let's do a simple network task, and find the common set of people followed on Twitter by two of the best tennis players in the world. I have done the first bit of data collection for you:

```{r, eval = FALSE}

player_accounts <- c("iga_swiatek", "RafaelNadal")
